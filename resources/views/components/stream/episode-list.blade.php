...
<div class="space-y-1">
    @foreach($episodes as $ep)
        <a href="{{ route('watch.show', [$animeId, $ep['number']]) }}?language={{ $language }}" class="flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 group
            ...
            <div
                ...
                <i
                    class="fas fa-play text-xs {{ (isset($currentEpisode['number']) && $currentEpisode['number'] === $ep['number']) ? 'text-indigo-400' : 'text-zinc-500 group-hover:text-white' }}"></i>
            </div>
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between gap-2">
                    ...
                    @if(isset($ep['duration']))
                        <span class="text-[10px] text-zinc-500">{{ $ep['duration'] }}</span>
                    @endif
                </div>
                <p class="text-xs text-zinc-500">Episode {{ $ep['number'] }}</p>
            </div>
        </a>
    @endforeach
</div>